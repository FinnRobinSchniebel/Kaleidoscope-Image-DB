...
func ProcessZip(fileHeader *multipart.FileHeader, ruleLayers []string, fileLayer string, groupingIndex int) (int, map[int][]imageset.CollisionResponsePair, error) {

	//check array lengths and Grouping level index in range
	if len(ruleLayers) > 11 {
		return fiber.StatusBadRequest, nil, fmt.Errorf("Too many folder Layers.")
	}
	if groupingIndex > len(ruleLayers) {
		return fiber.StatusBadRequest, nil, fmt.Errorf("Grouping index is out of bounds. Please select a valid group index")
	}
	...
	if len(ruleLayers) == 0 {
		return fiber.StatusBadRequest, nil, fmt.Errorf("No rule layers provided, Cannot attain any info")
	}
	if filepath.Ext(fileHeader.Filename) != ".zip" {
		return fiber.StatusBadRequest, nil, fmt.Errorf("Invalid file type. Only .zip allowed")
	}
	...
	if err != nil {
		return fiber.StatusInternalServerError, nil, fmt.Errorf("Failed to download Zip. Check server disk space.")
	}
	defer RemoveTempZip(pathName)

	//unzip the zip for better access
	err = Unzip(pathName, imageset.BackendVolumeLocation+"/Temp")

	if err != nil {
		return fiber.StatusInternalServerError, nil, fmt.Errorf("zip could not be Processed: %s", err.Error())
	}
	folderPathName := strings.TrimSuffix(fileHeader.Filename, filepath.Ext(fileHeader.Filename))
	defer func() {
		err := RemoveFolder(imageset.BackendVolumeLocation + "/Temp/" + folderPathName)
		if err != nil {
			log.Print(err)
		}
	}()

	cont, err := ValidateAndParseZip(pathName, ruleLayers, fileLayer, groupingIndex)
	if err != nil {
		return fiber.StatusBadRequest, nil, fmt.Errorf("failed to parse files: %s", err.Error())
	}

	ISets, Skipped, Errors, err := createImageSetsFromParsedZipData(cont)

	//log.Print(cont, err)
	log.Print("Sets Print: ")

	for i := range ISets {
		imageset.PrintISet(ISets[i])
	}

	log.Print(Skipped, Errors)
...
func createImageSetsFromParsedZipData(parsedDataMap map[string][]ParsedFolderInfo) ([]imageset.ImageSetMongo, []string, []string, error) {

	var skippedList []string
	var errorList []string
	var err error
	var result []imageset.ImageSetMongo
	...
	for groupingKey := range parsedDataMap {
		...
		for dataIndex := range parsedDataMap[groupingKey] {
			...
			if parsedDataMap[groupingKey][dataIndex].FileType == ".txt" {
				disc, err := readTxtAsDiscription(parsedDataMap[groupingKey][dataIndex].File)
				if err != nil {
...
func readTxtAsDiscription(file *zip.File) (string, error) {
